jQuery(document).ready(function ($) {
    ...
    var table = $("#drugUtilizationAnnualReportTable").DataTable({
        ...
        initComplete: function () {
            ...
            var row = $('#drugUtilizationAnnualReportTable tfoot tr');
            row.find('th').each(function () {
                $(this).css('padding', 8);
            });
            $('#drugUtilizationAnnualReportTable thead').append(row);
            ...
            $('div#drugUtilizationAnnualReportTable_info').html($('div#drugUtilizationAnnualReportTable_info').html().replace(/\(filtered from (.*?) total entries\)/, ""));
        }
    });
    //Filter data, just show one type of record
    table.column(2).search('Drug Utilization Report (DUR)').draw();
});
